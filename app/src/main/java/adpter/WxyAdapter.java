...
public class WxyAdapter extends BaseQuickAdapter<WxyBean, BaseViewHolder> {
    ...
    protected void convert(BaseViewHolder helper, WxyBean item) {
        ...
        helper.setText(R.id.wxy_qyname, "企业名称：" + item.getQyname());

        if (mycode.equals("home")) {
            helper.getView(R.id.wxy_qyname).setVisibility(View.VISIBLE);
        } else {
            helper.getView(R.id.wxy_qyname).setVisibility(View.GONE);
        }
